...
TH1F statistiche::createHistogram() const
{
    // Determina valore massimo, valore minimo e bin
    double min = minimumValue();
    double max = maximumValue();
    int n_bin = floor(sqrt(last_index_m)) + 1;

    // Definizione istogramma root
    TH1F hist("hist", "Distribuzione del campione", n_bin, min, max);
