/* Si inserisca il codice sorgente dell’esercizio precedente in un ciclo che effettui il confronto al variare del numero di eventi considerato per il fit, da 20 a 10000, con andamento regolare in scala logaritmica.
* Si utilizzino diversi TGraphErrors per disegnare l’andamento dei parametri e delle loro incertezze al variare del numero di eventi, per i due tipi di stimatore.
    ...
    for (int i_Nmax = v_eventi.size () ; i_Nmax > 10 ; i_Nmax /= 2)
      {
        ...
        double min = floor (minimoVec(v_eventi)) ;
        double max = ceil (massimoVec(v_eventi)) ;
        int N_bin  = round (sqrt (i_Nmax)) ;
        N_bin = massimoTra(N_bin, 10) ;
